...
class WebDriverAgent {
  constructor (xcodeVersion, args = {}, fbsimctl = true) {
    ...
    if (!fbsimctl) {
      this.testrunner = new XcodeBuild(this.xcodeVersion, this.device, {
      ...
      this.testrunner = new FBSimctl(this.device.udid, args.bundleId, this.wdaLocalPort || WDA_AGENT_PORT);
    }
  }
  ...
  async launch (sessionId) {
    ...
    if (_.isFunction(this.testrunner.ensureRunPossible)) {
      await this.testrunner.ensureRunPossible();
    }
    ...
    await this.testrunner.init(this.noSessionProxy);
